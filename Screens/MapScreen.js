...
function MapScreen(props) {
    ...
    var UploadPlaceImageToCloudinary = await fetch("http://172.16.188.156:3000/upload-user-picture", {
      method: 'post',
      body: data
    });
    ...
    if (responseFromCloudinary.imageSaved) {
        ...
        var addPinToDB = await fetch ('http://172.16.188.156:3000/newplace', {
            method: 'POST',
    ...
    useEffect(() => {
        async function askPermissions() {
            ...
            var request = await fetch(`http://172.16.188.156:3000/places`);
